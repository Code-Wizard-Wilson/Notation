...
export function Tooltip({
  ...
  useLayoutEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      ...
      if (side === "right") {
        ...
        const minTop = inset;
        const maxTop = Math.max(minTop, window.innerHeight - inset - height);
        const clampedTop = Math.min(Math.max(centeredTop, minTop), maxTop);
        setShiftX(0);
        setShiftY(clampedTop - centeredTop);
        return;
      }

      const container = anchor.closest<HTMLElement>(".editor-pane, .desktop-navigation, [role='dialog']");
      const bounds = container?.getBoundingClientRect() ?? { left: 0, right: window.innerWidth };
      const minLeft = bounds.left + inset;
      const maxLeft = Math.max(minLeft, bounds.right - inset - width);
      ...
      setShiftX(clampedLeft - centeredLeft);
      setShiftY(0);
    });
  ...
  const lift = side === "top" ? 7 : side === "bottom" ? -7 : 0;
  const slideX = side === "right" ? -7 : 0;
        ...
        <AnimatePresence>
          {open && (
            <motion.span
              ...
              initial={
                reduced
                  ...
                  : skipped
                    ...
                    : { opacity: 0, scale: 0.9, x: slideX, y: lift, filter: "blur(4px)" }
              }
              ...
              style={{
                transformOrigin: side === "top" ? "50% 100%" : side === "bottom" ? "50% 0%" : "0% 50%",
                left: shiftX,
                top: side === "right" ? shiftY : undefined,
              }}
