...
export function Drawer({
  ...
  const edge =
    side === "right"
      ? "right-3 rounded-[18px] border"
      : "left-3 rounded-[18px] border";
  ...
  const tree = (
    ...
    >
      <motion.div
        ...
        className="absolute inset-0 bg-black/18 backdrop-blur-[1.5px]"
      />
      <motion.div
        ref={drawer.panelRef}
        ...
        className={`absolute top-3 bottom-3 flex flex-col overflow-hidden border-stone-200/90 bg-white/98 shadow-[0_24px_70px_-28px_rgba(24,22,20,0.38),0_2px_10px_rgba(24,22,20,0.06)] outline-none ${edge} ${
      ...
      >
        <header
          ...
          className={`flex select-none items-start gap-3 border-b border-stone-200/80 px-5 py-4 ${
        ...
        >
          <div className="min-w-0 flex-1">
            <h2
              id={titleId}
              className="truncate text-[13px] font-medium text-stone-800"
            >
              {title}
            </h2>
            {description ? (
          ...
          <button
            ...
            className="-mr-1 grid size-8 shrink-0 place-items-center rounded-[9px] text-stone-400 outline-none transition-colors duration-150 hover:bg-stone-100 hover:text-stone-800 focus-visible:bg-[#C5F367] focus-visible:text-stone-900"
          >
            {CLOSE_ICON}
          </button>
        </header>
